...
def agent_unified_check(combined_input, full_text_for_search, api_key, model_name):
    ...
    ### ⚠️ 全域邊界規則 (嚴格遵守)
    1. **分區獨立**：本文件分為上半部 [SUMMARY_TABLE] 與下半部 [DETAIL_TABLE]。
    2. **互不干涉**：明細表(dimension_data) 的數據 **絕對禁止** 從總表(Summary) 抓取。即使明細表缺數據，也不可以去總表借用「實交數量」或「申請數量」。
    
    ### 1. 明細表數據 (來源: 下半部 === [DETAIL_TABLE] ===)
    *請僅在下半部規格區塊搜尋數據，忽略上半部表格。*
    ...
    - **batch_total_qty**: (預設為 `0`)
      ...
      - **禁令**: 若標題**不含**上述關鍵字(例如只是車修、銲補)，**一律填 `0`**。**絕對禁止**填入總表(Summary)的「實交數量」或「申請數量」。
